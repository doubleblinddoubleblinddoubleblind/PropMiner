...
fopEvaluationAccuracy= fopHeteroCPExp + 'RQ4/eval_changeBeamSize/'
...
for i in range(0,len(listOfProjects)):
    ...
    try:
        for j in range(0,len(lstFpFile)):
            ...
            lstBeamsizes=[10,20,50,100,200,500,1000]
            for beamsizeIndex in range(0,len(lstBeamsizes)):
                beamsize=lstBeamsizes[beamsizeIndex]
                vectorSizeSE=15
                vectorSizeSUM=10

                dictCoseOrigin,mrrOrgSE,bleuOrgSE,top1AccOrgSE,top5AccOrgSE=performEvaluationOnCodeSearch(dictVectorsOfItemsForEvaluation,beamsize,fopProjectEvaluationAccuracy+'beam_{}/'.format(beamsize))
                dictCosumOrigin,mrrOrgSUM,bleuOrgSUM,top1AccOrgSUM,top5AccOrgSUM=performEvaluationOnCodeSummarization(dictVectorsOfItemsForEvaluation, beamsize, fopProjectEvaluationAccuracy+'beam_{}/'.format(beamsize))
                # print(dictCoseAug)
                # mix2DictToExcelFile(dictCoseOrigin,dictCoseAug,fopProjectEvaluationAccuracy+'cose_compare.csv')
                dictCoseAug,mrrAugSE,bleuAugSE,top1AccAugSE,top5AccAugSE=performEvaluationOnCodeSearchWithNLPTDimensions(dictVectorsOfItemsForEvaluation,vectorSizeSE, beamsize, fopProjectEvaluationAccuracy+'beam_{}/'.format(beamsize), fopNLPTNewDimensionResult)
                dictCosumAug,mrrAugSUM,bleuAugSUM,top1AccAugSUM,top5AccAugSUM=performEvaluationOnCodeSummarizationWithNLPTDimensions(dictVectorsOfItemsForEvaluation,vectorSizeSUM, beamsize,fopProjectEvaluationAccuracy+'beam_{}/'.format(beamsize), fopASTNewDimensionResult)
