...
def performEvaluationOnCodeSearchWithNLPTDimensions(dictVectorsOfItemsForEvaluation,augVectorSize, beamsize, fopProjectEvaluationAccuracy,fopNewDimensionResult):
    listKeys=list(dictVectorsOfItemsForEvaluation.keys())
    halfBeam=beamsize//2
    lstTopKForEachInstances=[]
...
def performEvaluationOnCodeSummarizationWithNLPTDimensions(dictVectorsOfItemsForEvaluation,augVectorSize, beamsize,
...
fopEvaluationAccuracy= fopHeteroCPExp + 'RQ3/eval_changeAugSize/'
...
for i in range(0,len(listOfProjects)):
    ...
    try:
        for j in range(0,len(lstFpFile)):
            ...
            lstDimensionAugSizeCose=range(1,25)
            lstDimensionAugSizeCosum =range(1,25)
            for dimensionSizeIndex in range(0,len(lstDimensionAugSizeCose)):
                vectorSizeSE=lstDimensionAugSizeCose[dimensionSizeIndex]
                vectorSizeSUM = lstDimensionAugSizeCosum[dimensionSizeIndex]

                # dictCoseOrigin,mrrOrgSE,bleuOrgSE=performEvaluationOnCodeSearch(dictVectorsOfItemsForEvaluation,vectorSizeSE,beamsize,fopProjectEvaluationAccuracy)
                # dictCoseAug,mrrOrgSE,bleuOrgSE=performEvaluationOnCodeSummarization(dictVectorsOfItemsForEvaluation,vectorSizeSUM, beamsize, fopProjectEvaluationAccuracy)
                # print(dictCoseAug)
                # mix2DictToExcelFile(dictCoseOrigin,dictCoseAug,fopProjectEvaluationAccuracy+'cose_compare.csv')
                dictCoseAug,mrrAugSE,bleuAugSE,top1AugSE,top5AugSE=performEvaluationOnCodeSearchWithNLPTDimensions(dictVectorsOfItemsForEvaluation,vectorSizeSE, beamsize, fopProjectEvaluationAccuracy+'{}/'.format(vectorSizeSE), fopNLPTNewDimensionResult)
                dictCosumAug,mrrAugSUM,bleuAugSUM,top1AugSUM,top5AugSUM=performEvaluationOnCodeSummarizationWithNLPTDimensions(dictVectorsOfItemsForEvaluation,vectorSizeSE, beamsize,fopProjectEvaluationAccuracy+'{}/'.format(vectorSizeSE), fopASTNewDimensionResult)
                ...
                lstCoseTop1Aug.append(top1AugSE)
                lstCosumTop1Aug.append(top1AugSUM)
                lstCoseTop5Aug.append(top5AugSE)
                lstCosumTop5Aug.append(top5AugSUM)
                f1=open(fopProjectEvaluationAccuracy+'totalMRR-BLEU.txt','w')
                f1.write('MRR-COSE\t'+'\t'.join(map(str,lstCoseMRRAug))+'\n')
                f1.write('MRR-COSUM\t' + '\t'.join(map(str, lstCosumMRRAug)) + '\n')
                f1.write('BLEU-COSE\t'+'\t'.join(map(str,lstCoseBLEUAug))+'\n')
                f1.write('BLEU-COSUM\t' + '\t'.join(map(str, lstCosumBLEUAug)) + '\n')
                f1.write('Top1-COSE\t' + '\t'.join(map(str, lstCoseTop1Aug)) + '\n')
                f1.write('Top1-COSUM\t' + '\t'.join(map(str, lstCosumTop1Aug)) + '\n')
                f1.write('Top5-COSE\t' + '\t'.join(map(str, lstCoseTop5Aug)) + '\n')
                f1.write('Top5-COSUM\t' + '\t'.join(map(str, lstCosumTop5Aug)) + '\n')
